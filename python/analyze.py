...
def parse_args():
    ...
    parser.add_argument(
        "--num_days", default=1, type=int, help="number of days back to plot"
    )
...
for log in ["log.0.json", "log.json"]:
    with open(log) as f:
        for line in f:
            ...
            if (now - dt).total_seconds() > 60 * 60 * 24 * args.num_days:
                continue
            # print(
            #     dt,
...
if last_time is not None and last_start is not None:
